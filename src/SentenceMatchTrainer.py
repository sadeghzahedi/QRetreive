...
def evaluate(dataStream, valid_graph, sess, outpath=None,
    ...
    return MAP_MRR(scores, labels, flag_valid
                               ,sent1s, sent2s, atts ,word_vocab)

def MAP_MRR(logit, gold, flag_valid, sent1s, sent2s, atts, word_vocab):
    Tot_question = len (logit) / 10.0
    print ("Tot_q", len (logit), Tot_question)
    c_1_j = 0.0 #map
    c_2_j = 0.0 #mrr
    visited = 0
    output_sentences = []
    output_attention_weights = []
    for i in range(int(Tot_question)):
        prob = logit[visited: visited + 10]
        label = gold[visited: visited + 10]
        if flag_valid == True:
            question = sent1s[visited: visited + 10]
            answers = sent2s[visited: visited + 10]
            if FLAGS.store_att == True:
                attention_weights = atts [visited: visited + 10]
        visited += 10
    ...
    my_map = c_1_j/Tot_question
    my_mrr = c_2_j/Tot_question
    if flag_valid == False:
...
def Generate_random_initialization(cnf):
    if FLAGS.is_random_init == True:
        ...
        if FLAGS.context_layer_num == 2:
             context_lstm_dim = [20] #[x for x in range(50, 110, 10)]
        else:
             context_lstm_dim = [100]#[150, 200]#[x for x in range(50, 160, 10)]
        #
        ...
        FLAGS.pooling_type = random.choice(pooling_type)
...
def main(_):
    ...
    log_dir_pre = FLAGS.model_dir + 'wik'
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    path_prefix = log_dir + "/SentenceMatch.{}".format(FLAGS.suffix)
    path_prefix_pre = log_dir_pre + "/SentenceMatch.{}".format(FLAGS.suffix)
    ...
    while (Get_Next_box_size(FLAGS.start_batch) == True):
        ...
        #             if not var.name.startswith("Model"): continue
                ...
                with tf.Session() as sess:
                    ...
                    if FLAGS.has_pretrain == True:
                        print("Restoring model from " + FLAGS.pretrain_path)
                    ...
                    for step in range(max_steps):
                        ...
                        _, loss_value, _score = sess.run([train_graph.get_train_op(), train_graph.get_loss()
                                                  , train_graph.get_score()], feed_dict=feed_dict)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_box',default=False, type= bool, help='do we have cuda visible devices?')
    ...
    parser.add_argument('--sampling',default=False, help='for loss back prop')
    ...
    parser.add_argument('--pooling_type', default=1, help='Fix pre-trained word embeddings during training.', action='store_true')
