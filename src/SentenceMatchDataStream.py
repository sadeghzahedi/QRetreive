...
def wikiQaGenerate(filename, label_vocab, word_vocab, char_vocab, max_sent_length, batch_size, is_training,
    ...
    for key in list(question_dic):
        if len(question_dic[key]["question"]) < 10:
            print (question_dic[key]["question"][0])
        question_count += 1
...
def make_indices(sentence1, word_vocab, char_vocab, max_sent_length):
    item = re.split("\t", sentence1)
    word_idx = []
    char_matrix_idx = []
    for i in range (len (item)):
        word_idx_1 =word_vocab.to_index_sequence(item[i])
        char_matrix_idx_1 = char_vocab.to_character_matrix(item[i])
        ...
        word_idx.append (word_idx_1)
        char_matrix_idx.append(char_matrix_idx_1)
    return word_idx, char_matrix_idx


def make_box (word_idx1, word_idx2, char_matrix_idx1, char_matrix_idx2):
    box_word_idx1 = [] # question sentences in box
    box_word_idx2 = [] # passage sentences in box
    box_char_matrix_idx1 = []
    box_char_matrix_idx2 = []
    for i in range (len (word_idx1)):
        for j in range (len (word_idx2)):
            box_word_idx1.append((word_idx1[i])[:])
            box_word_idx2.append((word_idx2[j])[:])
            box_char_matrix_idx1.append((char_matrix_idx1[i])[:])
            box_char_matrix_idx2.append((char_matrix_idx2[j])[:])
    return box_word_idx1, box_word_idx2, box_char_matrix_idx1, box_char_matrix_idx2

def make_idx (label_vocab, label, word_vocab, sentence1, sentence2, char_vocab, max_sent_length, is_as):
    label_id = label
    word_idx1, char_matrix_idx1 = make_indices(sentence1, word_vocab, char_vocab,max_sent_length)
    word_idx2, char_matrix_idx2 = make_indices(sentence2, word_vocab, char_vocab,max_sent_length)
    word_idx_1, word_idx_2, char_matrix_idx_1, char_matrix_idx_2 = \
        make_box(word_idx1, word_idx2, char_matrix_idx1, char_matrix_idx2)
    ...
    def __init__(self, inpath, word_vocab=None, char_vocab=None, POS_vocab=None, NER_vocab=None, label_vocab=None, batch_size=60, 
       ...
       # inha vase pointwise: (badan pak kon)
        ...
        for batch_index, (batch_start, batch_end) in enumerate(batch_spans):
            ...
            def char_idx_length (char_matrix_idx):
                ans = []
                for x in (char_matrix_idx):
                    l = []
                    for y in x:
                        l.append(len(y))
                    ans.append(l)
            ...
            for i in range(batch_start, batch_end):
                ...
                sent1_length_batch.append([len(cur_word_idx) for cur_word_idx in word_idx_1])
                sent2_length_batch.append([len(cur_word_idx) for cur_word_idx in word_idx_2])
                sent1_char_length_batch.append(char_idx_length (char_matrix_idx_1))
                sent2_char_length_batch.append(char_idx_length(char_matrix_idx_2))
                if POS_vocab is not None:
            ...
            _word_idx_1_batch = []
            _word_idx_2_batch = []
            _char_matrix_idx_1_batch = []
            _char_matrix_idx_2_batch = []
            _sent1_length_batch = []
            _sent2_length_batch = []
            _sent1_char_length_batch = []
            _sent2_char_length_batch = []
            for i in range (len(sent1_length_batch)): #len = 10
                max_sent1_length = np.max(sent1_length_batch[i]) #max len of box i (a a a b b b)
                max_sent2_length = np.max(sent2_length_batch[i]) # (c d e c d e)
                max_char_length1 = np.max([np.max(aa) for aa in sent1_char_length_batch[i]])
                if max_char_length1>max_char_per_word: max_char_length1=max_char_per_word

                max_char_length2 = np.max([np.max(aa) for aa in sent2_char_length_batch[i]])
                if max_char_length2>max_char_per_word: max_char_length2=max_char_per_word
            
                _word_idx_1_batch.append(pad_2d_matrix(word_idx_1_batch[i], max_length=max_sent1_length))
                _word_idx_2_batch.append(pad_2d_matrix(word_idx_2_batch[i], max_length=max_sent2_length))

                _char_matrix_idx_1_batch.append(pad_3d_tensor(char_matrix_idx_1_batch[i], max_length1=max_sent1_length, max_length2=max_char_length1))
                _char_matrix_idx_2_batch.append(pad_3d_tensor(char_matrix_idx_2_batch[i], max_length1=max_sent2_length, max_length2=max_char_length2))

                _sent1_length_batch.append(np.array(sent1_length_batch[i]))
                _sent2_length_batch.append(np.array(sent2_length_batch[i]))

                _sent1_char_length_batch.append(pad_2d_matrix(sent1_char_length_batch[i], max_length=max_sent1_length))
                _sent2_char_length_batch.append(pad_2d_matrix(sent2_char_length_batch[i], max_length=max_sent2_length))


            word_idx_1_batch = tuple(_word_idx_1_batch)
            word_idx_2_batch = tuple(_word_idx_2_batch)
            char_matrix_idx_1_batch = tuple(_char_matrix_idx_1_batch)
            char_matrix_idx_2_batch = tuple(_char_matrix_idx_2_batch)
            sent1_length_batch = tuple(_sent1_length_batch)
            sent2_length_batch = tuple(_sent2_length_batch)
            sent1_char_length_batch = tuple(_sent1_char_length_batch)
            sent2_char_length_batch = tuple(_sent2_char_length_batch)
