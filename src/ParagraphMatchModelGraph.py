...
class ParagraphMatchModelGraph(object):
    def __init__(self, num_classes, word_vocab=None, char_vocab=None, POS_vocab=None, NER_vocab=None,
                 ...
                 is_answer_selection=True, is_shared_attention=True, modify_loss=0, is_aggregation_lstm=True,
                 max_window_size=3
                 , prediction_mode='list_wise', context_lstm_dropout=True, is_aggregation_siamese=True,
                 unstack_cnn=True, with_context_self_attention=False,
        ...
        self.question_lengths = get_place_holder(q_count, tf.int32, [None, None]) #[10, sc] 10:candidate question count sc:sentece count
        self.passage_lengths = get_place_holder(q_count, tf.int32, [None, None]) #[10, sc]
        self.truth = get_place_holder(q_count, tf.float32,
                                          [None]) #[10]
        self.mask = get_place_holder(q_count, tf.float32, [None, None, None]) #[10, sc, max_sent_length]
        self.in_question_words = get_place_holder(q_count, tf.int32, [None,
                                                                      None, None])  # [10, sc, max_sent_length]
        self.in_passage_words = get_place_holder(q_count, tf.int32, [None,
                                                                     None])   # [10, sc, max_sent_length]
        ...
        with tf.variable_scope('ParagraphModel'):
            for i in range(q_count):
                ...
                question_len = input_shape[1] #sc
                input_shape = tf.shape(self.in_passage_words[i])
                passage_len = input_shape[1] #sc
                ...
                in_question_repres = tf.concat(in_question_repres, 2) # [10, sc, senlen, word_dim]
                in_passage_repres = tf.concat(in_passage_repres, 2)  # [10, sc, senlen, word_dim]
                ...
                mask = tf.sequence_mask(self.passage_lengths[i], passage_len,
                                        dtype=tf.float32)  # [10, sc, senlen]
                question_mask = tf.sequence_mask(self.question_lengths[i], question_len,
                                                 dtype=tf.float32)  # [10, sc, senlen]
                ...
                if with_output_highway == False:
                    ...
                    logits = match_utils.highway_layer(in_val=tf.expand_dims(match_representation, 0),
                                                       input_size=match_dim, output_size=match_dim, with_highway=True
                ...
                if prediction_mode != 'point_wise':
                    ...
                    if prediction_mode == 'list_wise':
                        ...
                        if new_list_wise == False:
                            ...
                            if top_treshold < 0:
                                ...
                                hinget = tf.reshape(self.hinge_truth[i],
                                                    [self.answer_count[i], self.answer_count[i]])  # [a, a]
                    ...
                    gold_matrix = tf.one_hot(g1_matrix, num_classes,
                                             dtype=tf.float32)  # gold_matrix = tf.one_hot(self.truth, num_classes)
                    loss_list.append(
                        tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=gold_matrix)))
    ...
    attention_weights = property(get_attention_weights, set_attention_weights, del_attention_weights,
                                 "prob's docstring")
